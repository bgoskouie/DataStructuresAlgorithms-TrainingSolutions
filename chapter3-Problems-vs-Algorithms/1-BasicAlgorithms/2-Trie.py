...
class Trie(object):
    ...
    def add(self, word):
        ...
        if word == '':
            return
        trie = self.root.children
        print(f"~~~~~~~~~~~~{word}~~~~~~~~~~~~")
        for char in word:
            if char in trie.keys():
                print(f"char {char} is in trie")
            else:
                print(f"char {char} is not in trie -- adding")
                trie.update({char: {'word_end': False}})
            trie = trie[char]
        trie.update({'word_end': True})
        print(f"trie: {self.root.children}")
...
#word_list = ['goo', 'good', 'goodbye', 'goods', 'goodwill', 'gooses']
word_trie = Trie()
